...
(function() {
    ...
    angular.module('home').factory('dataservice', dataservice);

    dataservice.$inject = ['$http', '$q', '$timeout', 'breeze', 'logger'];
    function dataservice($http, $q, $timeout, breeze, logger) {

        var serviceName = '/breeze/qwerty'; // route to the same origin Web Api controller
        ...
        function getUserN(uid, pwd) {
            var query = breeze.EntityQuery
                .from("users")
                .where("active", "==", true);

            if (uid.length) { // if userid sent
                query = query.where("userid", "==", uid);
            }

            if (pwd.length) { // if pwd sent
