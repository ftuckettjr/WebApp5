...
(function() {
    ...
    function controller($q, $scope, $timeout, dataservice, logger) {
        ...
        vm.users = [{ userid: 'test', pwd: 'pwd1' }, { userid: 'car', pwd: 'pwd2' }];
        vm.userFilter = userFilter;
        vm.userFilterText = "";
        vm.userid = "a";
        vm.pwd = "b";
        vm.userLookup = userLookup;
        ...
        function userFilter(todoItem) {
            // Beware: this is called a lot!
            var userFilterText = vm.userFilterText;
            return userFilterText ?
                // if there is search text, look for it in the description; else return true
                -1 != todoItem.Description.toLowerCase().indexOf(userFilterText.toLowerCase()) :
                true;
        };

        function userLookup() {
            //editEnd();
            // wait for Ng binding, then proceed
            $timeout(getUsersImpl, 0);

            function getUsersImpl() {
                dataservice.getUserN(vm.userid, vm.pwd)
                    .then(querySucceeded)
                    .catch(queryFailed);
            }
